...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public' AS $$
    SELECT EXISTS (
        SELECT 1 FROM public.user_roles WHERE user_id = _user_id AND role IN ('admin', 'manager', 'support')
    )
$$;
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public' AS $$
    SELECT EXISTS (
        SELECT 1 FROM public.user_roles WHERE user_id = _user_id AND role IN ('admin', 'manager', 'support', 'viewer')
    )
$$;
...
DECLARE
    t text;
BEGIN
    FOREACH t IN ARRAY ARRAY['profiles','categories','products','product_variants','collections','customers','customer_addresses','orders','banners','coupons','shipping_rules','pickup_locations','search_filters','showcase_sections','theme_settings','store_settings','payment_settings','home_blocks']
    LOOP
